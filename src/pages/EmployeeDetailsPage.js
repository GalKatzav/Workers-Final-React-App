...
const EmployeeDetailsPage = () => {
  ...
  return (
      <div className="employee-details">
          <img src={picture.large} alt={`${name.first} ${name.last}`} className="employee-image" />
          <h3>
          ...
          <p>Phone: {phone}</p>
          <p>Address: {`${location.street.number} ${location.street.name}, ${location.city}, ${location.state}, ${location.country}, ${location.postcode}`}</p>
          <div className="map-container">
              <MapContainer center={[location.coordinates.latitude, location.coordinates.longitude]} zoom={13} scrollWheelZoom={false}>
                  <TileLayer
